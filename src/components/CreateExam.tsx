...
import { Sparkles, Upload, FileText, Camera, X, Loader2, Printer, ChevronLeft, FileDown } from 'lucide-react';
...
export const CreateExam: React.FC = () => {
  ...
  const handleExportWord = () => {
    ...
    questions.forEach((_ , pageIndex) => {
      ...
      const pageQuestions = questions.slice(pageIndex * 15, (pageIndex + 1) * 15);
      pageQuestions.forEach((q, i) => {
        content += `
          <tr>
            <td style="text-align: center;">${pageIndex * 15 + i + 1}</td>
            <td style="font-size: 16pt;">${q.question} ${q.hint ? `<i>(※${q.hint})</i>` : ''}</td>
            <td style="text-align: center; color: #e11d48; font-weight: bold; font-size: 18pt;">${quizMode === 'teacher' ? q.answer : ''}</td>
            <td></td>
          </tr>
        `;
      });
    ...
    link.href = url;
    link.download = `國語文練習卷_${new Date().toISOString().slice(0,10)}.doc`;
            ...
            >
              ...
              {pageIndex === 0 ? (
                <div className="text-center border-b-4 border-slate-800 pb-4 mb-6">
                  <h1 className="text-4xl font-serif font-black mb-4 tracking-[0.3em] text-slate-900">國語文能力診斷練習卷</h1>
                ...
                <table className="w-full border-collapse text-lg font-serif border border-slate-300">
                  <thead>
                    <tr className="bg-slate-50 print:bg-gray-100 border-b-2 border-slate-800">
                      <th className="p-3 w-14 text-center font-black text-slate-600 border-r border-slate-300">#</th>
                      ...
                      .map((q, i) => (
                        <tr key={i} className="border-b border-slate-300">
                          ...
                          <td className="py-3 px-4 font-serif text-xl align-middle leading-tight text-slate-800 border-r border-slate-300">
                            {q.question}
                            {q.hint && <span className="text-slate-400 text-sm italic ml-2">※{q.hint}</span>}
                          </td>
                          <td className="py-3 px-4 bg-slate-50/10 align-middle text-center min-h-[60px] border-r border-slate-300">
                            {quizMode === 'teacher' && (
